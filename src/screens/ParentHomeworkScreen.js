...
function HomeworkCard({ assignment, index }) {
  const router = useRouter();
  const params = encodeURIComponent(JSON.stringify(assignment));
  const isOverdue = assignment.status === "Overdue";
  const [backgroundColor, accentColor] =
    homeworkBoxColors[index % homeworkBoxColors.length];
  return (
    <View style={[styles.card, { backgroundColor, borderColor: accentColor }]}>
      <View style={styles.cardHeader}>
        <View style={styles.subjectWrap}>
          <View style={styles.subjectIcon}>
            <Icon name="book-outline" size={16} color={colors.blue} />
          </View>
          <View style={styles.subjectCopy}>
            <Text style={styles.subject}>{assignment.subject}</Text>
            <Text style={styles.topic}>{assignment.topic}</Text>
          </View>
        </View>
        <View
          style={[
            styles.statusBadge,
            isOverdue ? styles.overdueBadge : styles.activeBadge,
          ]}
        >
          <Text
            style={[
              styles.statusText,
              isOverdue ? styles.overdueText : styles.activeText,
            ]}
          >
            {assignment.status}
          </Text>
      ...
      <View style={styles.dateRow}>
        <View>
          ...
          <Text style={styles.value}>{assignment.assignedDate}</Text>
        </View>
        <View>
      ...
      <View style={styles.actionRow}>
        ...
        <Pressable
          ...
          onPress={() => router.push(`/homework-details?assignment=${params}`)}
        >
          <Text style={styles.outlineButtonText}>Details</Text>
        </Pressable>
        <Text style={[styles.label, styles.actionLabel]}>Status / Action</Text>
        <Pressable
          style={styles.primaryButton}
          onPress={() =>
            router.push(`/homework-submission?assignment=${params}`)
          }
        >
...
export default function ParentHomeworkScreen({ session, onSessionExpired }) {
      ...
      .catch((requestError) => {
        setError(
          requestError instanceof ApiError
            ...
            : "Unable to load homework. Please try again later.",
        );
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={[styles.introTitle, { color: colors.purpleAccent }]}>
          Homework & Assignments
        </Text>
        <Text style={styles.introText}>
          Track, preview and submit active homework
        </Text>
      </View>
      ...
      {loading ? (
        ...
        >
          ...
          <Pressable onPress={loadAssignments}>
...
const styles = StyleSheet.create({
  ...
  introTitle: { color: colors.ink, fontSize: 17, fontWeight: "900" },
  introText: { color: colors.muted, fontSize: 12, marginTop: 5 },
  ...
  subjectWrap: { flex: 1, flexDirection: "row", alignItems: "center", gap: 9 },
  subjectIcon: {
    width: 32,
    height: 32,
    borderRadius: 9,
  ...
  subjectCopy: { flex: 1 },
  subject: { color: colors.blue, fontSize: 11, fontWeight: "900" },
  topic: { color: colors.ink, fontSize: 14, fontWeight: "900", marginTop: 3 },
  statusBadge: {
    borderRadius: 999,
    borderWidth: 1,
    paddingHorizontal: 8,
  ...
  activeBadge: { backgroundColor: colors.paleGreen, borderColor: "#B6E7D0" },
  activeText: { color: colors.green },
  overdueBadge: { backgroundColor: colors.paleOrange, borderColor: "#F5D98B" },
  overdueText: { color: colors.orange },
  dateRow: {
    flexDirection: "row",
    justifyContent: "space-between",
    borderTopWidth: 1,
  ...
  value: { color: colors.ink, fontSize: 12, fontWeight: "800", marginTop: 3 },
  actionRow: {
    flexDirection: "row",
    alignItems: "center",
    flexWrap: "wrap",
  ...
  outlineButtonText: { color: colors.blue, fontSize: 10, fontWeight: "900" },
  primaryButton: {
