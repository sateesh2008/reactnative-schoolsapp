        ...
        <View style={styles.modal} onStartShouldSetResponder={() => true}>
          ...
          {options.map((option) => (
            <Pressable
              ...
              style={[
                styles.modalOption,
                option === value && styles.modalOptionActive,
              ]}
              onPress={() => {
                onSelect(option);
                onClose();
...
export default function TeacherAttendanceScreen({ session, onBack }) {
  ...
  const loadAttendance = async () => {
    ...
    try {
      setData(
        normalizeData(
          await teacherAttendanceApi.getAttendance(context, session),
        ),
      );
    } catch (requestError) {
      setData(emptyData);
  ...
  useEffect(() => {
    loadAttendance();
  }, [date, className, section, subject]);
  ...
  const runCutoff = async () => {
    ...
    try {
      await teacherAttendanceApi.runAttendanceCutoff(context.date, session);
      setData((current) => ({
        ...current,
        records: current.records.map((record) =>
  ...
  const loadHistory = async () => {
    try {
      const records = await teacherAttendanceApi.getHistory(
  ...
  useEffect(() => {
    if (activeTab === "History Log") loadHistory();
  }, [activeTab]);
  ...
  const exportCsv = async () => {
    ...
    const rows = [
      ...
      ...data.records.map((record) => [
        ...
        subject,
        record.name,
        record.roll || record.admissionNumber || record.studentId,
        record.status,
      ]),
    ];
    try {
      await Share.share({
        ...
        message: rows.map((row) => row.map(escapeCsv).join(",")).join("\n"),
      });
    } catch {
      Alert.alert(
        "Export unavailable",
        "Unable to open the Android share sheet.",
      );
    }
  };
  ...
  const renderDailyLog = () => (
    <>
      <View style={styles.filterCard}>
        ...
        >
          <Text style={styles.selectText}>Select an option (Class)</Text>
        ...
        >
          <Text style={styles.selectText}>Select an option (Sections)</Text>
          <Icon name="chevron-down" size={16} color={colors.muted} />
        </Pressable>
      </View>
  ...
  return (
    <View style={styles.container}>
          ...
          <View>
            ...
            <View style={styles.exportCard}>
              ...
              <Pressable style={styles.primaryButton} onPress={exportCsv}>
                <Icon name="download-outline" size={17} color={colors.white} />
      ...
      <SelectModal
        ...
        value={className}
        options={["Select an option", ...data.classes]}
      ...
      <SelectModal
        ...
        value={section}
        options={["Select an option", ...data.sections]}
        onSelect={setSection}
        onClose={() => setSelecting(null)}
      />
    </View>
