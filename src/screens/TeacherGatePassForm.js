...
function ActionButton({ title, onPress, secondary, disabled }) { return <Pressable accessibilityRole="button" hitSlop={8} disabled={disabled} style={[styles.button, secondary && styles.secondaryButton, disabled && styles.disabledButton]} onPress={onPress}><Text style={[styles.buttonText, secondary && styles.secondaryButtonText]}>{title}</Text></Pressable>; }

export default function TeacherGatePassForm({ visible, students, classes, loading, onClose, onSubmit }) {
  ...
  const studentOptions = students.filter((student) => !className || String(student.className) === String(className));
