    ...
    <View style={[styles.card, item.status === 'Approved' ? styles.approvedCard : item.status === 'Rejected' ? styles.rejectedCard : styles.pendingCard]}>
      ...
      {item.status === 'Pending' ? (
        <View style={styles.actions}>
          <Pressable style={styles.approveButton} onPress={() => onApprove(item)}>
  ...
  return (
    <View style={styles.field}>
      ...
      <Pressable style={styles.select} onPress={onPress}>
        <Text style={styles.selectText}>{value.toLocaleDateString('en-GB')}</Text>
...
export default function TeacherLeaveManagementScreen({ session }) {
  ...
  const normalizedRecords = useMemo(
    () =>
      records.map((item, index) => ({
        ...item,
        id: String(item.id || item.leave_id || item.leaveId || `${item.studentName || 'leave'}-${index + 1}`),
        ...
        studentName: item.studentName || item.student_name || item.student || 'Student',
        leaveType: item.leaveType || item.leave_type || item.category || 'Medical Leave',
  ...
  const filtered = useMemo(
    () =>
      normalizedRecords.filter((item) => {
        ...
        return (
          (!search || [item.studentName, item.parentName, item.reason].some((value) => value.toLowerCase().includes(search))) &&
  ...
  const pending = normalizedRecords.filter((item) => item.status === 'Pending').length;
  const approved = normalizedRecords.filter((item) => item.status === 'Approved').length;
  const rejected = normalizedRecords.filter((item) => item.status === 'Rejected').length;
  ...
  const updateStatus = async (item, nextStatus) => {
    try {
      ...
      setRecords((current) =>
        current.map((record) =>
          String(record.id || record.leave_id || record.leaveId) === String(item.id)
  ...
  const submitRequest = async () => {
    ...
    try {
      const created = await teacherApi.createLeaveRequest(
        {
          ...
          from: request.from.toLocaleDateString('en-GB'),
          to: request.to.toLocaleDateString('en-GB'),
      ...
      setRecords((current) => [
        {
          ...created,
          id: created.id || created.leave_id || created.leaveId || `leave-${current.length + 1}`,
      ...
      setRequest({ category: 'Medical Leave', from: new Date(), to: new Date(), reason: '', notes: '' });
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.topTabs}>
        <Pressable
          style={[styles.topTab, section === 'Request absence' && styles.activeTab]}
          onPress={() => setSection('Request absence')}
        >
          <Text style={styles.tabText}>Request absence</Text>
        </Pressable>
        <Pressable
          style={[styles.topTab, section === 'Pending Queue' && styles.activeTab]}
      ...
      <View style={styles.summaryGrid}>
        <Summary title="Pending Queue" value={pending} label="REQUIRES ACTION" icon="time-outline" tint="#FFF1DF" accent={colors.orange} />
        <Summary title="Approved Today" value={approved} label="SYNCED SUCCESS" icon="checkmark-circle-outline" tint="#E2F4EE" accent={colors.green} />
        <Summary title="Rejection Rate" value={rejected} label="AUDIT DENIED" icon="close-circle-outline" tint="#FDECEC" accent={colors.red} />
        <Summary title="Total Volume" value={records.length} label="SYSTEM HISTORY" icon="list-outline" tint={colors.paleBlue} accent={colors.blue} />
      </View>
      ...
      {section === 'Request absence' ? (
        <View style={styles.requestBox}>
          ...
          <Select
            ...
            onChange={(value) => setRequest((current) => ({ ...current, category: value }))}
          />

          <DateField label="Start date" value={request.from} onPress={() => setDateField('from')} />
          {dateField === 'from' ? (
            <DateTimePicker
              value={request.from}
              mode="date"
          ...
          <DateField label="End date" value={request.to} onPress={() => setDateField('to')} />
          {dateField === 'to' ? (
            <DateTimePicker
              value={request.to}
              mode="date"
          ...
          <View style={styles.field}>
            ...
            <TextInput
              ...
              onChangeText={(value) => setRequest((current) => ({ ...current, reason: value }))}
              multiline
          ...
          <View style={styles.field}>
            ...
            <TextInput
              value={request.notes}
              onChangeText={(value) => setRequest((current) => ({ ...current, notes: value }))}
              multiline
              style={styles.textarea}
      ...
      {section === 'Pending Queue' ? (
        <View style={styles.listWrap}>
          <View style={styles.filterGrid}>
            ...
            <Select label="Leave type" value={type} options={types} onChange={setType} />
          </View>
          ...
          {loading ? (
            ...
            <Text style={styles.emptyText}>No leave requests match the selected filters.</Text>
          )}

          <View style={styles.pagination}>
            <Pressable disabled={currentPage <= 1} onPress={() => setPage((value) => Math.max(1, value - 1))}>
              <Text style={[styles.pageButton, currentPage <= 1 && styles.pageDisabled]}>Prev</Text>
            </Pressable>
            <Text style={styles.pageMeta}>Page {currentPage} / {totalPages}</Text>
            <Pressable disabled={currentPage >= totalPages} onPress={() => setPage((value) => Math.min(totalPages, value + 1))}>
              <Text style={[styles.pageButton, currentPage >= totalPages && styles.pageDisabled]}>Next</Text>
            </Pressable>
          </View>
        </View>
      ) : null}
      ...
      {selectedLeave ? (
        <Modal transparent animationType="fade" visible onRequestClose={closeLeaveDetails}>
          <Pressable style={styles.backdrop} onPress={closeLeaveDetails}>
            <View style={styles.modal} onStartShouldSetResponder={() => true}>
              <Text style={styles.modalTitle}>Leave Details</Text>
              <Text style={styles.modalText}>Student: {selectedLeave.studentName}</Text>
              <Text style={styles.modalText}>Parent: {selectedLeave.parentName}</Text>
              <Text style={styles.modalText}>Type: {selectedLeave.leaveType}</Text>
              <Text style={styles.modalText}>From: {selectedLeave.from}</Text>
              <Text style={styles.modalText}>To: {selectedLeave.to}</Text>
              <Text style={styles.modalText}>Reason: {selectedLeave.reason}</Text>
              <Text style={styles.modalText}>Status: {selectedLeave.status}</Text>
...
const styles = StyleSheet.create({
  ...
  topTab: { flex: 1, minHeight: 44, borderWidth: 1, borderColor: colors.line, borderRadius: 8, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center' },
  ...
  summary: { width: '48%', minHeight: 120, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 11, padding: 11 },
  summaryIcon: { width: 30, height: 30, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  summaryTitle: { color: colors.ink, fontSize: 11, fontWeight: '900', marginTop: 8 },
  summaryValue: { color: colors.ink, fontSize: 22, fontWeight: '900', marginTop: 3 },
  summaryLabel: { color: colors.muted, fontSize: 9, marginTop: 4 },
  requestBox: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 12, padding: 13, marginBottom: 16 },
  ...
  status: { fontSize: 10, fontWeight: '900', textTransform: 'uppercase', paddingVertical: 4, paddingHorizontal: 8, borderRadius: 999, backgroundColor: '#F4F7F7', color: colors.ink },
  approved: { backgroundColor: '#E2F4EE', color: colors.green },
  ...
  approveButton: { flex: 1, backgroundColor: colors.green, borderRadius: 8, paddingVertical: 10, alignItems: 'center' },
  rejectButton: { flex: 1, backgroundColor: colors.red, borderRadius: 8, paddingVertical: 10, alignItems: 'center' },
  ...
  backdrop: { flex: 1, backgroundColor: 'rgba(10, 20, 22, 0.35)', justifyContent: 'flex-end' },
  options: { backgroundColor: colors.white, padding: 17, borderTopLeftRadius: 18, borderTopRightRadius: 18, maxHeight: '75%' },
  option: { paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.line },
  optionText: { color: colors.ink, fontSize: 13 },
  modal: { backgroundColor: colors.white, borderTopLeftRadius: 18, borderTopRightRadius: 18, padding: 20, maxHeight: '80%' },
  modalTitle: { color: colors.ink, fontSize: 18, fontWeight: '900', marginBottom: 10 },
  modalText: { color: colors.ink, fontSize: 13, marginBottom: 6 },
  closeButton: { marginTop: 14, backgroundColor: colors.blue, borderRadius: 8, paddingVertical: 10, alignItems: 'center' },
  closeText: { color: colors.white, fontWeight: '800' },
  loadingText: { color: colors.muted, fontSize: 12, textAlign: 'center', marginTop: 12 },
  emptyText: { color: colors.muted, fontSize: 12, textAlign: 'center', marginTop: 12 },
});
