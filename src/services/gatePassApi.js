...
const dateTimeForApi = (value) => {
  ...
  const isoLike = raw.replace(/\s+/g, " ").replace(/\//g, "-");

  if (/^\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}$/.test(isoLike)) {
    return isoLike.replace(" ", "T");
  }

  if (/^\d{2}-\d{2}-\d{4}[T\s]\d{2}:\d{2}$/.test(isoLike)) {
    const [datePart, timePart] = isoLike.split(/[T\s]/);
    const [day, month, year] = datePart.split("-");
    return `${year}-${month}-${day}T${timePart}`;
  }

  if (/^\d{4}-\d{2}-\d{2}$/.test(isoLike)) {
    return `${isoLike}T00:00`;
  }

  if (/^\d{2}-\d{2}-\d{4}$/.test(isoLike)) {
...
export const gatePassApi = {
  ...
  async getStudents(classId, session) {
    const payload = await apiRequest("/students", {
      token: session?.token,
      query: { class_id: classId, status: "Active" },
    });
  ...
  async create(input, session) {
    const body = {
      ...
      out_time: normalizeGatePassTime(
        firstValue(
          input?.outTime,
          input?.out_time,
          input?.exitDateTime,
    ...
    const payload = await apiRequest("/gate-pass", {
      method: "POST",
      token: session?.token,
      body,
    });
  ...
  async exit(id, session) {
    return this.updateStatus(id, { status: "CHECKED_OUT" }, session);
  },
  ...
  async cancel(id, session) {
    return this.updateStatus(id, { status: "CANCELLED" }, session);
  },
